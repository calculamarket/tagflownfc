...
import { getTag, upsertTag } from "@/lib/tags.functions";
...
function ConfigurePage() {
  ...
  useEffect(() => {
    getTag({ data: { id } })
      .then((t) => {
        setTag(t);
        if (t) {
          if (t.destination_type === "pix") {
            const d = (t.destination ?? {}) as Record<string, string>;
            setPixKey(d.key ?? "");
            setPixName(d.merchant_name ?? "");
            setPixCity(d.city ?? "");
            setPixAmount(d.amount ?? "");
            setMode("pix");
          } else if (t.destination_type === "links") {
            setItems(parseLinkItems((t.destination ?? {}) as Record<string, unknown>));
            setMode("links");
          }
        }
      })
  ...
  const saveTag = async (destination_type: "pix" | "links", destination: Record<string, string>) => {
    if (!tag) return;
    setSaving(true);
    try {
      await upsertTag({
      ...
      setSaved(true);
      toast.success("Etiqueta configurada!");
    } catch (e) {
  ...
  const saveLinks = () => {
    ...
    saveTag("links", { items: JSON.stringify(clean) });
  };
  ...
  return (
    <div className="p-6 lg:p-10 max-w-2xl mx-auto space-y-6">
      ...
      {mode === "pix" && (
        <div className="rounded-lg border border-border bg-card p-5 space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Chave PIX" value={pixKey} onChange={setPixKey} placeholder="CPF, e-mail, telefone ou aleatória" />
            <Field label="Nome do recebedor" value={pixName} onChange={setPixName} placeholder={tag.name} />
            <Field label="Cidade" value={pixCity} onChange={setPixCity} placeholder="SAO PAULO" />
            <Field label="Valor (opcional)" value={pixAmount} onChange={setPixAmount} placeholder="0,00" />
          </div>
          <Button disabled={saving} onClick={savePix}>
          ...
          {saved && pixKey.trim() && (
            <PixResult
              tagUrl={tagUrl}
              brcode={buildPixPayload({ key: pixKey.trim(), name: pixName.trim() || tag.name, city: pixCity.trim(), amount: pixAmount.trim() })}
            />
          )}
        </div>
      )}
      ...
      {mode === "links" && (
        <div className="rounded-lg border border-border bg-card p-5 space-y-4">
          <LinksMiniBuilder items={items} onChange={setItems} />
          <Button disabled={saving} onClick={saveLinks}>
            {saving ? "Salvando…" : "Salvar menu de links"}
          </Button>
...
/** Construtor compacto do menu de links. */
...
function PixResult({ tagUrl, brcode }: { tagUrl: string; brcode: string }) {
  ...
  useEffect(() => {
    if (!brcode) return;
    QRCode.toDataURL(brcode, { width: 600, margin: 1, errorCorrectionLevel: "M" })
      .then(setStaticQr).catch(() => {});
  }, [brcode]);
    ...
    <div className="space-y-4 rounded-lg bg-muted/40 p-4">
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Ao escanear com a câmera, cai direto na tela de pagamento PIX.
        </p>
      </div>
      ...
      {brcode && (
        <div className="space-y-2 border-t border-border pt-3">
          ...
          <p className="text-xs text-muted-foreground">
            Este QR é fixo (chave/valor definidos agora) — imprima para o cliente pagar escaneando
            com o próprio app do banco. Ele não muda se você reconfigurar a etiqueta depois.
          </p>
        </div>
      )}
    </div>
