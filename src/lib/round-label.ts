...
const QR_RATIO = 0.72; // fração do diâmetro ocupada pelo QR quadrado
...
/**
 * Abre uma folha A4 pronta para impressão com várias etiquetas redondas do
 * mesmo QR, cada uma no diâmetro informado (mm). Empacota o máximo por folha.
 */
export function openRoundLabelSheet(pngDataUrl: string, diamMm: number, quantity: number): void {
...
<title>Etiquetas redondas ${diamMm} mm</title><style>
@page { size: A4 portrait; margin: 8mm; }
* { box-sizing: border-box; }
body { margin: 0; }
.head { font-family: system-ui, sans-serif; font-size: 12px; color: #555; padding: 0 0 4mm; }
.wrap { font-size: 0; }
.cell { display: inline-block; width: ${diamMm}mm; height: ${diamMm}mm; margin: 1.5mm; page-break-inside: avoid; }
.cell img { width: 100%; height: 100%; display: block; border-radius: 50%; }
@media print { .head { display: none; } }
</style></head><body>
<div class="head">Etiquetas ${diamMm} mm — Ctrl/Cmd+P para imprimir ou salvar em PDF. Imprima em tamanho real (100%).</div>
